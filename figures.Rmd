...
- sort out R^2 stuff
- add FC/FCC ??
- tweak supp maps (clarify scales - log or orig scale?)
...
png("figure1.png", width=12, height = 8, units = "in", res=300)
# plot layout
m_layout <- layout(matrix(1:9, ncol=3, byrow=TRUE))
mktitle   <- function(lab) axis(side=3, at=0, lab=lab, tck=0, lty=0, cex.axis=1.6, font=2)
...
map_plot <- function(x, x_orig, title = "") {
  ...
  mktitle(title)
  ## legend
  par(bty='n'); llab <- round(range(x_orig, na.rm=TRUE))
...
png("figure1_supp.png", width=8, height = 5.333, units = "in", res=300)
# plot layout
m_layout <- layout(matrix(1:4, ncol=2, byrow=TRUE))
map_plot(ecoreg$NPP_log_sc, ecoreg$NPP_mean, "NPP (mean)")
map_plot(ecoreg$NPP_cv_inter, ecoreg$NPP_cv_sc, "NPP (variability)")
map_plot(ecoreg$Feat_log_sc, ecoreg$Feat_mean, "Fire consumption (mean)")
map_plot(ecoreg$Feat_cv_inter, ecoreg$Feat_cv_sc, "Fire consumption (variability)")
invisible(dev.off())
```
...
mkplot <- function(var, colour, type = "rect", xlab = "", title= "") {
  ...
  if (type == "rect") {
    ...
    g1 <- g1 + geom_point(aes(x={{var}}, y = mid), colour = colour, size=3) +
      geom_segment(x=0, aes(xend = {{var}}, y = mid, yend = mid), colour = colour, size=1)
...
library(colorspace)
do_plot <- function(plotdata, preddata) {
  ggplot(plotdata, aes(focal, richness, colour = bshort))  +
...
```{r colors, fig.width = 20, fig.height = 20, warning=FALSE, message=FALSE}
...
plot_grid(
    ...
    fig4 + scale_colour_discrete_sequential() + noleg + ggtitle("colorspace, sequential"),
