...
save_both <- function(fn, ...) {
    ggsave(..., file = paste0(fn, ".pdf"))
...
save_both("fig2",width=14, height=8)
```

Same thing but as boxplots (sorted by latitude??)
```{r plot_fig2box, fig.width=14, fig.height=5.5, warning=FALSE}
e2 <- (ecoreg
    |> as_tibble()
    ## n.b. fct_reorder uses median by default
    |> full_join(shortnames, by = "biome")
    |> mutate(across(bshort, forcats::fct_reorder, .fun = \(x) mean(abs(x)),
                     .x = y))
    |> dplyr::select(biome, bshort, plants, mamph, mbirds, mmamm, NPP_mean, Feat_mean)
)
levels(e2$biome)

area_wid <- (ecoreg
    |> group_by(biome)
    |> summarise(y = mean(abs(y)), area_km2 = sum(area_km2)/1e6)
    |> full_join(shortnames, by = "biome")
    |> arrange(y)
)

adj_widths <- function(g1, rel_widths) {
    g1B <- ggplot_build(g1)
    newdat <- g1B$data[[1]]
    wids <- mean(newdat$new_width)*rel_widths/mean(rel_widths)
...
mkbplot <- function(var, colour, leftmost = FALSE, xlab = "", title= "", rel_widths = NULL) {
  g1 <- ggplot(e2)
  g1 <- g1 + geom_boxplot(aes(x={{var}}, y = bshort), fill = colour)
  if (leftmost) {
  ...
  g1 <- g1  + labs(x=xlab, title= title) +
      theme(axis.title.y = element_blank()) +
      theme(plot.title = element_text(hjust = 0.5))
  if (is.null(rel_widths)) return(g1)
  adj_widths(g1, rel_widths)
}

plot_list <- list(
    mkbplot(NPP_mean, cols[1], leftmost = TRUE, xlab=expression(g~C/m^{2}/year), title="NPP"),
    ##            rel_widths = area_wid$area_km2)
...
wrap_plots(plot_list, nrow=1, widths=c(1.2, rep(1,4)))
save_both("fig2X",width=14, height=5.5)
```
...
save_both("fig3", width=8, height = 6)
```
...
save_both("fig3X", width=8, height = 6)
```
...
do_plot <- function(plotdata, preddata, interax = FALSE,
                    log_scale = TRUE,
                    qvec = c(0.1, 0.5, 0.9)) {
    ...
    if (log_scale) {
        gg1 <- gg1 + scale_x_log10(labels = scientific_10) +
            scale_y_log10()
    }
    gg1
}
...
print(fig4)
save_both(plot = fig4, fn = "fig4", width = 8, height = 8)
```
...
print(fig5)
save_both(plot = fig5, fn = "fig5", width = 6.5, height = 8)
```

## Figure 4 (no log)

```{r fig4_nolog, fig.width = 8, fig.height = 8, warning = FALSE}
print(fig4_nolog)
save_both(plot = fig4_nolog, fn = "fig4_nolog", width = 6.5, height = 8)
```

```{r fig5_nolog, fig.width = 8, fig.height = 8, warning = FALSE}
print(fig5_nolog)
save_both(plot = fig5_nolog, fn = "fig5_nolog", width = 8, height = 8)
```
...
print(fig3a_supp)
save_both(fn="fig3a_supp", width=10, height=5)
```
...
print(fig3b_supp)
save_both(fn="fig3b_supp", width=10, height=5)
```
...
trans_names <- . %>%    mutate(across(taxon, factor,
                  levels = c("mamph_log", "mbirds_log", "mmamm_log"),
                  labels = c("Amphibians", "Birds", "Mammals"))) %>% 
    mutate(across(term, factor,
                  levels = c("NPP_log_sc", "Feat_log_sc"),
                  labels = c("NPP", "Fire consumption")))
...
print(interax_plot)
save_both("fig_interax_supp", width = 20, height = 12)
```
...
save_both("fig_robust_supp", width = 20, height = 8)
```
...
## spatial autocorr plots

```{r refit0}
res <- readRDS("allfits_sum_gamm4.rds")
taxa <- unique(res$coefs$taxon)
best_names <- purrr::map(taxa, get_best_name, x=res) %>% setNames(taxa)
```

```{r refit, cache=TRUE}
## refit best gamm models -- something dropped?
source("fit_utils.R")
newfits <- list(
    mbirds = fit_all("mbirds_log", single_fit = c(1, 1, 2), platform = "gamm4", add_sos = TRUE),
    mamph = fit_all("mamph_log", single_fit = c(1, 1, 3), platform = "gamm4", add_sos = TRUE),
    mmamm = fit_all("mmamm_log", single_fit = c(1, 2, 1), platform = "gamm4", add_sos = TRUE))
```

```{r plot_spat, fig.keep = "none"}
get_plotdata <- function(x) {
    pp <- plot(x$gam, scheme = 4, pch = 1)
    return(pp[[1]])
}
pd <- lapply(newfits, get_plotdata)
```

```{r plot_spat2, results = "hide", fig.width = 14, fig.height = 6}
my_plot <- function(x, main = "") {
    with(x, {
        m <- matrix(fit, 40, 40)
        p <- raw
        image(y, x, t(m), useRaster = TRUE)
        points(p$y, p$x)
        ## x-axis -180 to 180
        ## y-axis -50 to 70
    })
...
tnms <- c("birds", "amphibians", "mammals")
op <- par(mfrow = c(2,2), las = 1, mar = c(4,4,3,3))
invisible(Map(my_plot, pd, tnms))
dev.copy(pdf,file="fig_supp_autocorr.pdf", width = 12, height=8)
dev.off()
dev.copy(png,file="fig_supp_autocorr.png", units = "in", width = 12, height=8,
         res = 300)
dev.off()
```
...
library(patchwork)
library(ggplotify)
ld <- function(x, title = "hello") {
    r <- lattice::dotplot(ranef(x), as.table = TRUE)$flor_realms
    r$main <- title
    as.ggplot(r)
}
pl <- Map(ld, newfits, tnms)
(pl[[1]] + pl[[2]]) / pl[[3]]
dev.copy(pdf,file="fig_supp_realms.pdf", width = 10, height=10)
dev.off()
dev.copy(png,file="fig_supp_realms.png", units = "in",
