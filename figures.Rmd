...
do_maps <- FALSE ## skip map drawing sometimes (slow!)
```
...
```{r get_data, eval=do_maps, warning=FALSE}
...
## NOT idempotent
all_rsq_gamm4_sgv_2 <- (all_rsq_gamm4_sgv
  %>% filter(taxon != "plants_log")  ## snuck in
  %>% mutate(across(taxon, factor,
             ## FIXME: cross-check order with coeff plot
             levels = c("mbirds_log", "mamph_log", "mmamm_log"),
             labels = c("Birds", "Amphibians", "Mammals")))
...
p2 <- ggplot(coefs,aes(estimate,term,colour=sig))+
  ...
  ## don't squash panels together (messes with tick labels);
  ## shrink y-axis tick labels
...
taxon_names <- paste(c("Birds","Amphibians","Mammals"),"(species/km^2)",sep="~")
names(taxon_names) <- paste0(c("mbirds","mamph","mmamm"),"_log")
...
plotdata <- purrr::map_dfr(remef_vars, ~ .[["data"]]) %>% replace_names()
...
do_plot <- function(plotdata, preddata) {
  plot_alldat <- ggplot(plotdata, aes(focal, richness, colour = biome))  +
...
```{r fig5, fig.width = 8, fig.height = 8}
print(fig5)
