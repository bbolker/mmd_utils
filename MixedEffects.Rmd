...
Best non-singular model (by AIC) is `r best_name` (`r best_df` parameters).
...
cc <- confint(best_model,method=ci_method)
...
best_coef$nppcv_fac <- factor(best_coef$nppcv_var,
                              labels=c("non-NPPcv terms",
                                       "NPPcv terms"))
ggplot(best_coef,aes(estimate,term,
    ...
    geom_vline(xintercept=0,lty=2)+
    facet_wrap(~nppcv_fac,scale="free")+
    labs(x="",y="")
```
...
**FIXME**: make sure that this graph adapts to different focal taxa!!
...
pdata$mbirds_log[pdata$mbirds_log< pred_lower_lim] <- NA
## these predictions do *not* incorporate random effects (biome, flor_realms)
gg0 <- ggplot(ecoreg,aes(NPP_log,mbirds_log))+
...
print(gg0 + geom_point(aes(colour=biome,shape=flor_realms)))
```
...
if (length(rem1)==nrow(ecoreg)) {
    ...
    print(gg0 %+% ecoreg +  ## incorporate new column 
          geom_point(aes(y=rem1,colour=biome,shape=flor_realms)))  ## change response var
}
```

```{r}
ecoreg$rem2 <- fix_NAs(remef(best_model,ran="all",
                             fix=c("NPP_log","NPP_cv_ctr","Feat_cv_ctr"),grouping=TRUE),
                       best_model)
ecoreg$rem3 <- fix_NAs(remef(best_model,ran="all",
                             fix=c("NPP_log","NPP_cv_ctr","Feat_log"),grouping=TRUE),
                       best_model)
## FIXME: drop dimensions on Feat_cv_ctr upstream
gg1 <- ggplot(ecoreg,aes(c(Feat_cv_ctr),rem3))+
...
- start writing!
- rescale CV parameters; consider rescaling all parameters; consider smart vs brute force approaches
- more/nicer $R^2$ plots?
- fit models without biome/realm interaction
- combined coefficient plots from multiple taxa
- handy-dandy functions to simplify making remef plots with specified response variable etc.?
