...
The manuscript in progress is [here](https://docs.google.com/document/d/1cW7Yw_bLQnf1R_BUTExNCrSCXC4fzAZCG4p-DpJkm0U/edit).
...
What about a combined "fire $R^2$"?
...
get_fire_R2 <- function(taxon, method="sgv", clust.id=NULL) {
    ...
    beta <- fixef(model)
    if (is.null(clust.id)) {
        clust.id <- tail(names(model@flist),1)
    }
...
```{r calc_fire_R2,echo=FALSE}
## watch out for factor vs char!
R2_vals <- purrr::cross_df(list(taxon=c("mamm","bird","amph"),
                       method=c("sgv","nsj"),
                       clust.id=c("biome_FR","biome","flor_realms")))
R2_list <- list()
for (i in seq(nrow(R2_vals))) {
    res <- with(R2_vals[i,],
                tibble(taxon=taxon,method=method,clust.id=clust.id,
                       get_fire_R2(taxon,method,clust.id)))
    ## with(res[1,],cat(taxon,method,clust.id,Rsq,"\n"))
    R2_list <- c(R2_list,list(res))
}
all_R2 <- bind_rows(R2_list) %>% mutate(effect=factor(effect,levels=c("fire","nonfire","model")))
```
...
Bottom line: results don't depend very much on method, are reasonable. But overall magnitude *does* depend
on which cluster is chosen (why does this affect NSJ method ...?)
...
(all_R2
    %>% filter(method=="sgv",clust.id=="flor_realms")
    %>% dplyr::select(taxon,effect,Rsq,upper.CL,lower.CL)
    %>% rename(lwr="lower.CL",upr="upper.CL",estimate="Rsq")
    %>% transmute(taxon,effect,
                  est=sprintf("%1.2g (%1.2g, %1.2g)",estimate,lwr,upr))
