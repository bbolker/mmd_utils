...
```{r cmp_R2_2}
library(Matrix)
get_fire_R2 <- function(taxon, method="sgv", which_clust=NULL) {
    ...
    beta <- fixef(model)
    if (is.null(which_clust)) {
        which_clust <- length(model@flist)
    }
    clust.id <- names(model@flist)[which_clust]
    ...
    r2_model <- cmp_R2(c=C_model, x=X, SigHat=SigHat, beta=beta, method = method,
                       obsperclust=obsperclust, nclusts=nclusts)
    r2_fire <- cmp_R2(c=C_fire, x=X, SigHat=SigHat, beta=beta, method = method,
                      obsperclust=obsperclust, nclusts=nclusts)
    R2 <- data.frame(effect=c("model","fire"),
                     rbind(r2_model,r2_fire))
    R2 = within(R2, {
...
R2_vals <- expand.grid(taxon=c("mamm","bird","amph"),
                       method=c("sgv","nsj"),
                       which_clust=c("biome_FR","biome","flor_realms"))
R2_list <- list()
for (i in seq(nrow(R2_vals))) {
    R2_list <- c(R2_list,list(with(R2_vals[i,],
                                   tibble(taxon=taxon,method=method,clust=which_clust,
                                          get_fire_R2(taxon,method,which_clust)))))
}
all_R2 <- bind_rows(R2_list)
ggplot(all_R2,aes(Rsq,clust,colour=effect,shape=method)) + facet_wrap(~taxon) +
