...
Can we re/unscale parameters? How?
(point estimate gets multiplied by scale

```{r ctx_rescaled, echo=FALSE}
rescale_fun <- function(x, name) {
    ss <- strsplit(name,":")[[1]]
    for (s in ss) {
        sc <- attr(ecoreg[[s]],"scaled:scale")
...
ctx_rescaled <- mutate(ctx_rescaled,
              type=case_when(
                  grepl("log",term_orig) & grepl("cv",term_orig) ~ "mixed",
                  grepl("log",term_orig) ~ "log",
                  grepl("cv",term_orig) ~ "cv"))
```

Order by 
```{r ctx_rescaled_plot,fig.width=8,fig.height=8}
std_order <- c("Area","NPP","Fire","NPP CV","Fire CV")
oo <- outer(std_order[-1],std_order[-1],paste,sep=":")
std_order2 <- c(std_order,oo[upper.tri(oo)])
...
```
```{r}
ctx_rescaled %>% dplyr::select(taxon,term,estimate,lwr,upr) %>% arrange(term,taxon)
```

TO DO: traditional order
Amph/Birds/Mammals?
check NPP CV
